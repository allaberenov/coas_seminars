...
UTF-8 (8-bit — «формат преобразования Юникода, 8-бит») — распространённый стандарт кодирования символов, позволяющий более компактно хранить и передавать символы Юникода, используя переменное количество байт (от 1 до 4), и обеспечивающий полную обратную совместимость с 7-битной кодировкой ASCII. 
...
## Почему не перейти просто в UTF вместо ASCII?

1. ASCII встроен в историю и железо

ASCII появился в 1960-х и стал базовым стандартом для протоколов, файловых форматов.

Практически всё «железо» и софт (старые ОС, BIOS, терминалы, протоколы) исходно ожидали именно 1 байт = 1 символ.

ASCII лёг в основу многих кодировок → «вырвать» его было бы больно.


2. ASCII идеально совместим с UTF-8

UTF-8 специально сделан так, чтобы первые 128 символов (0x00–0x7F) совпадали с ASCII.
→ Старые программы, которые понимают ASCII, автоматически могут читать UTF-8, если там только латиница.
Это облегчило миграцию.
